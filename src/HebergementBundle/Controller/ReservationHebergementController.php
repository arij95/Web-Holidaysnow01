...
*/
 /*   /**
       ...
       // $reservation->setIdagence($hebergement->getIdagence());
        ...
        if($hebergement->getNombreChambre()>= $reservation->getNbrplace()){
            ...
            $reservation->setIdagence($hebergement->getIdagence());
              //  $reservation->setTypechambre('chambresingle');
                $choix = $form->get('typechambre')->getData();
            $choix1 = $form->get('tauxchambre')->getData();
                if ($choix == 'Chambresingle' && $choix1=='tauxComplet'){
                $reservation->setPrix($hebergement->getPrixSingle()*$reservation->getNbrplace()*$hebergement->getTauxComplet());}
                elseif ($choix == 'Chambredouble'  && $choix1=='tauxComplet'){
                    $reservation->setPrix($hebergement->getPrixDouble()*$reservation->getNbrplace()*$hebergement->getTauxComplet());}
                elseif ($choix == 'Chambresingle'  && $choix1=='tauxDemi'){
                    $reservation->setPrix($hebergement->getPrixSingle()*$reservation->getNbrplace()*$hebergement->getTauxDemi());}
                elseif ($choix == 'Chambredouble'  && $choix1=='tauxDemi'){
                    $reservation->setPrix($hebergement->getPrixDouble()*$reservation->getNbrplace()*$hebergement->getTauxDemi());}
    ...
    public function affResAction()
    {
        ...
        $res= $this->getDoctrine()->getRepository('HebergementBundle:ReservationHebergement')->supress($idu);
        return $this->render("@Hebergement/hebergement/mesreservations.html.twig",array('reservationhergement'=>$res));
    }

    public function afficherHebergementuserrAction(){
        $hebergement= $this->getDoctrine()->getRepository(Hebergement::class)->findAll();
        return $this->render("@Hebergement/hebergement/afficheHebergementuserr.html.twig",array('hebergement'=>$hebergement));
